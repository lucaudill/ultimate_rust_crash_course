...
fn change(arg: &mut String) {
    if !arg.ends_with("s") {
	//*arg = arg + "s"; //couldn't figure out concatenation...
	arg.push_str("s") // this is what the hint suggests
    }
}
...
fn inspect(arg: &String) {
    ...
    if arg.ends_with("s") {
	//println!("plural");
	plurality="plural".to_string();
    } else {
	//println!("singular");
	plurality="singular".to_string();
    }
    println!("{} is {}", *arg, plurality)
}
